...
public class AiProjectService {
	...
	public ResponseEntity<ApiResponses> getQsListService(CommonReqModel model, ResponseBean response,
			String authToken) {
...
//				System.out.println(role);
			...
			if (userCount < subLimit) {
				...
				if (data != null) {
					...
					if ("S".equals(userSubType) && !"".equals(model.getResume())
							&& "".equals(model.getJob_description())) {
//							System.out.println(1);
						data2 = geminiService.askGeminiForResumeQuestions(model.getLevel(), model.getResume(),
								userSubType);
...
//							System.out.println(2);
						data3 = geminiService.askGeminiForJDQuestions(model.getLevel(), model.getJob_description(),
								userSubType);
...
//					return response.AppResponse("Success", null, List.of("What is Spring boot?","What is JAva"));
	...
	public ResponseEntity<ApiResponses> getQsListRoleBasedService(CommonReqModel model, ResponseBean response,
			String authToken) {
		List<String> data = null, data2 = null, data3 = null;
		try {
			if (authToken.isBlank() || authToken.isEmpty()) {
...
//					return response.AppResponse("Success", null, List.of("What is Spring boot?","What is JAva"));
	...
	public ResponseEntity<ApiResponses> getFeedbackService(CommonReqModel model, ResponseBean response,
			String authToken) {
...
//				data=geminiService.getDummyInterviewFeedback();
			if (data != null) {
				...
				String formattedTopics = Arrays.stream(model.getTopic().split("\\|")).map(String::trim)
	...
	public ResponseEntity<ApiResponses> ResumeParseService(MultipartFile file, ResponseBean response, String authToken)
			throws Exception {
		try {
	...
	public ResponseEntity<ApiResponses> ResumeJDParseService(MultipartFile file, String JD, ResponseBean response,
	...
	public ResponseEntity<ApiResponses> getAiRoadMapService(CommonReqModel model, ResponseBean response,
			String authToken) {
		Map<String, Object> data = null;
		try {
			if (authToken.isBlank() || authToken.isEmpty()) {
